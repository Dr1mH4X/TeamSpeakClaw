...
此外，TeamSpeakClaw 还通过 NapCat 适配器，让您可以通过 QQ 与机器人交互。通过 NapCat（基于 OneBot 11 协议），您可以在 QQ 私聊或群聊中直接控制 TeamSpeak 服务器，实现跨平台管理。

## ✨ 功能特性

- **🧠 自然语言交互**：告别繁琐的指令手册。直接说"播放周杰伦的歌"或"把那个捣乱的人踢出去"。
- **🛡️ 细粒度权限控制**：内置强大的权限系统。您可以为不同的 TeamSpeak 服务器组/用户组配置特定的技能权限（例如：仅允许管理员使用踢人功能，普通用户仅限点歌）。
- **🔌 灵活的技能系统**：
...
- **🤖 广泛的模型支持**：兼容 OpenAI 接口格式，轻松接入 DeepSeek、ChatGPT 等多种大模型。
...
- [NeteaseTSBot](https://github.com/yichen11818/NeteaseTSBot) - **参考了其 `voice-service` 的实现，基于 MIT 协议。**
- [NapCatQQ](https://github.com/NapNeko/NapCatQQ) - QQ 支持
