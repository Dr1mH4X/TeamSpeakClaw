...
| 字段 | 说明 |
|---|---|
| `content` | 系统提示词，定义机器人的角色、个性、回复规则等 |

系统提示词支持占位符替换：
- `{date}` — 会被替换为当前本地日期（如 `2026-07-09`），用于提示 LLM 注意信息时效性

默认值请参考 [prompts.toml](https://github.com/Dr1mH4X/TeamSpeakClaw/blob/main/examples/config/prompts.toml) 的 `[system].content` 字段。
