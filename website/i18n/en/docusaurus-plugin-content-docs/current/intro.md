...
- **🔌 Flexible Skill System (Skills)**:
    - **Music Control**: Supports three backends — `ts3audiobot` (controlling [TS3AudioBot](https://github.com/ZHANGTIANYAO1/TS3AudioBot-NetEaseCloudmusic-plugin) via private messages), `tsmusicbot` (controlling [TSMusicBot](https://github.com/ZHANGTIANYAO1/teamspeak-music-bot)), or `tsbot_backend` (controlling [NeteaseTSBot](https://github.com/yichen11818/NeteaseTSBot) via HTTP API, supporting NetEase Cloud Music and QQ Music). Features include requesting songs, skipping, searching, volume adjustment, and sound effect settings.
